...
class SnakeGame:
    def __init__(self, seed=0, board_size=12, silent_mode=True):
        ...
        if not silent_mode:
            ...
            # Load sound effects (if files exist)
            mixer.init()
            try:
    ...
    def step(self, action):
        ...
        if (row, col) == self.food: # If snake eats food, it won't pop the last cell. The food grid will be taken by snake later, no need to update board vacancy matrix.
            food_obtained = True
            self.score += 1 # Add 1 point to the score when food is eaten. Max score = board_size^2 - 1
            if not self.silent_mode and self.sound_eat:
                self.sound_eat.play()
        else:
            food_obtained = False
        ...
        if not done:
            ...
            if not self.silent_mode:
                if len(self.snake) < self.grid_size and self.sound_game_over:
                    self.sound_game_over.play()
                elif self.sound_victory:
    ...
    def _generate_food(self):
        if len(self.non_snake) > 0:
            food = random.choice(list(self.non_snake))
    ...
    def draw_snake(self):
        ...
        if len(self.snake) > 1:
            ...
            for i, (r, c) in enumerate(self.snake[1:], 1):
                ...
                # Use different colors for body segments
